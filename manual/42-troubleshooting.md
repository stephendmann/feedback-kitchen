...
| **Builder will not advance past Step 3, or will not save.** | Criterion percentage weights do not total exactly 100%. | Adjust individual criterion percentages until the indicator reads `✅ Weights total 100% — good to go!`. The blocking message names your current total. |
...
| **Moodle rejected my exported CSV worksheet.** | The file fed to the export was not the original worksheet downloaded from that assignment, so its participant identifiers do not match. | Download a fresh worksheet from that assignment, re-run **Export Moodle worksheet…** against it, and upload the result. |
...
| **Feedback draft is out of sync with selected grades.** | Extensive manual edits were typed into the feedback box, overriding dynamic template updates. | Click **↺ Regenerate** to discard manual edits and rebuild the draft from current grade choices. |
