...
Three things stop the save, and the draft survives all of them. Without a name or an ID, Feedback Kitchen declines and says so in an amber notice. If you dismissed the cohort setup, earlier in the session or during the save itself, the background save skips rather than reopening the dialog, and an amber notice tells you nothing was stored. And if the write itself fails, most often because browser storage is full, you get an amber notice and nothing is stored.
