...
Several conditions can leave the draft in place. Without a name or an ID, Feedback Kitchen declines the save and says so in an amber notice. With **Copy feedback**, if you dismissed cohort setup earlier in the session and no cohort was created, the background save skips and an amber notice says nothing was stored. **✓ Finalise & Export** reopens the setup dialog instead. If you close that dialog without starting a cohort, no student is stored and no notice appears, so open **View list** to check before moving on. If the write itself fails, most often because browser storage is full, you get an amber notice and nothing is stored.
