...
Column counts (2026-06-13, + FK-25 split: PR #39 shipped a **rubric-version** drift indicator that was built under the label "FK-12" but is a *different* feature from FK-12's card — carded as **FK-25 · Rubric-version drift indicator → Shipped**; FK-12 remains the unbuilt cohort-consistency/anchoring indicator (D-10)): Safe to implement now: 0 · Needs inspection: 0 · Backlog: 4 (FK-15 · FK-16 · FK-19 · FK-22) · Ready to document: 1 (FK-10, fully closed) · In progress: 0 · Shipped: 22 · others: 0. Next free card ID: FK-27. *(Latest: FK-19 **import shipped #47**, **export in review #48** (round-trip complete pending merge + live-Moodle check → card stays in Backlog). FK-21 #46, FK-12 #44 + docs #45, FK-13 #43, FK-26 #42 all shipped this cycle.)*
...
### FK-19 · Moodle offline-grading-worksheet round-trip (batch import/export) — IMPORT SHIPPED (#47); EXPORT IN REVIEW (#48)
> **Split status (2026-06-14):** **import half SHIPPED — PR [#47](https://github.com/stephendmann/feedback-kitchen/pull/47) merged** (`validateWorksheet`→`planImport`→`buildCohortImport` + scorer import UI + safety guards). **Export half IN REVIEW — PR [#48](https://github.com/stephendmann/feedback-kitchen/pull/48)**: pure `buildExportWorksheet` (fill Grade + `feedbackText`-only into the re-supplied original worksheet, BOM/CRLF preserved, `markerNotes` never read) + the "Export Moodle worksheet" UI (file-picker re-supply → download under the same filename). Both halves: ~61 tests, axe 0, runtime-verified (re-import-over-marked smoke test; export markerNotes-exclusion + BOM bytes). **Only remaining: the live-Moodle round-trip upload check** (one session validates both directions; not code-blocking). Card moves to Shipped once #48 merges + the live check passes. Scoping finding: export consumes the *original* worksheet, so #47's data model needed no change.
