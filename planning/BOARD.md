...
Column counts (2026-06-13, + FK-25 split: PR #39 shipped a **rubric-version** drift indicator that was built under the label "FK-12" but is a *different* feature from FK-12's card — carded as **FK-25 · Rubric-version drift indicator → Shipped**; FK-12 remains the unbuilt cohort-consistency/anchoring indicator (D-10)): Safe to implement now: 1 (FK-12 ambient *consistency* indicator) · Needs inspection: 0 · Backlog: 5 (FK-15 · FK-16 · FK-19 · FK-21 · FK-22) · Ready to document: 1 (FK-10) · In progress: 1 (FK-13 score-result live region — IN REVIEW, PR #43) · Shipped: 19 (+FK-26 modexport tests, PR #42 / GH issue #4 closed) · others: 0. Next free card ID: FK-27.
...
### FK-13 · Score-result live region + validation-convention note — IN REVIEW (PR #43)
- **Status:** implemented on `feat/focus-mode-criterion-workspace`, **PR [#43](https://github.com/stephendmann/feedback-kitchen/pull/43) open**, CI green (Build & Test pass, Vercel pass). Awaiting review/merge.
- **What shipped in the PR:** sr-only `#score-result-live` (`aria-live='polite'`, `aria-atomic`) announcing the recomputed score + grade (+ band label, late-penalty/override/fail riders) on `recalculate()`, **debounced 500 ms + de-duplicated**; the deliberate hard-invalid (`#grade-override`) vs soft-warn (per-criterion out-of-band) validation convention documented at both setters.
- **DoD evidence:** grade change announces to AT (runtime-verified on demo scorer; re-grade re-announces, clear empties); axe harness **Scorer 0 violations / 51 passes**; regression guard `js/score-result-live-region.test.js`; **Jest 227/227**. Maps to WCAG 2.1 AA 4.1.3.
- **Out of scope (held):** validation-model refactor → FK-15; the optional `aria-describedby` ride-along and the inline-hex-vs-class note → FK-16 were not taken (additive core only). **Priority:** P2. **Effort:** S.
- **On merge:** move to Shipped; run post-merge sync (prune branch, fast-forward main).
