...
- **Done 2026-06-12 (`9d3ad1f`):** fixed by **moving the header boundary, not the nav** — refinement on the card's recommended fix to preserve visual order (moving the nav after `</header>` would have dropped the rail below the title band). `<header>` now wraps only the positioning line; the rail is a sibling (containing block `#app`); title band + note wrapped in `<section aria-label="Assessment title">` (named section = region landmark). Same intent, zero visual reorder.
- **Validated:** scrolled-pin assertion **PASS** (scrollY 1500 → rail rect top 56px = sticky offset, visible; identical in dark mode — was rect top −619 pre-fix); scroll-spy highlights the correct section at depth; focus battery, expand/collapse-all, recalc→sticky chain green; harness 0 violations / 122 passes (`region` clean with the new section landmark); Jest 98/98. **Scrolled-pin assertion is now a permanent runtime-battery item** (scroll ≥1000px → rail rect top ≈ sticky offset and visible).
- **SHIPPED 2026-06-12:** PR [#24](https://github.com/stephendmann/feedback-kitchen/pull/24) merged (`9240ed4`); production verified serving the tightened header boundary, the rail outside `<header>`, and the Assessment-title section landmark. Rail pinning restored live.
- **Column:** Ready to document (2026-06-12). **Priority:** P1. **Effort:** S (actual: S).
