...
### FK-19 · Moodle offline-grading-worksheet round-trip (batch import/export) — IMPORT SHIPPED (PR #47); EXPORT CORE DONE
> **Split status (2026-06-14):** the **import half SHIPPED — PR [#47](https://github.com/stephendmann/feedback-kitchen/pull/47) merged** (full `validateWorksheet`→`planImport`→`buildCohortImport` pipeline + scorer import UI + safety guards; ~54 tests; re-import-over-marked smoke test passed). The **export half** is part-built: the pure core **`buildExportWorksheet`** (fill Grade + `feedbackText`-only into the original worksheet, BOM/CRLF preserved, `markerNotes` never read) is done + tested on branch **`feat/fk19-export`** (commit `e83a7fd`, stacked → rebased onto main; 7 tests; round-trip proven). **Remaining: the export UI slice** (an "Export Moodle worksheet" button → file-picker re-supply → download) **+ its PR + the live-Moodle round-trip check.** Card stays in Backlog until the export UI ships. Scoping finding: export consumes the *original* worksheet, so #47's data model needed no change.
