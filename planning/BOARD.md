...
### FK-19 · Moodle offline-grading-worksheet round-trip (batch import/export) — IMPORT HALF IN REVIEW (PR #47)
> **Split status (2026-06-14):** the **import half is in review as PR [#47](https://github.com/stephendmann/feedback-kitchen/pull/47)** (full `validateWorksheet`→`planImport`→`buildCohortImport` pipeline + the scorer import UI + safety guards; ~54 tests; runtime + re-import-over-marked smoke test passed). The **export half** (write grades + feedback back to a Moodle-uploadable worksheet with BOM/CRLF byte-preserve + `markerNotes` exclusion) is **NOT yet built** — it remains on this card after #47 merges. INS-10 Q5 partly closed: real export confirmed UTF-8 BOM + CRLF (byte-checked); live-upload round-trip is the only PR-verification left.
