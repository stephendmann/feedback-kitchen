...
- **Findings (☑ 2026-06-13, Phase-3 kickoff — full code read; engine + renderer are cleanly split, public API on `window.CohortInsights`):**

  **Q1 · Stats available — a flat per-cohort object, distribution-rich, with NO standing per-criterion or per-tutor breakdown.** `cohortMetrics(config, students)` (`cohort-insights.js:110–252`) is a pure function returning **23 fields**:
  - **Distribution / spread:** `mean_pre_penalty`, `mean_total` (post-penalty), `min_total`, `max_total`, `total_score_sd`, `total_score_range`, `scale_use_ratio` (range/100), `skew` (Fisher-Pearson, null if n<3), `kurtosis` (excess, null if n<4), `bimodality` (gap heuristic, `{flagged}` only if n≥6), `grade_bands` (tier histogram: excellent/proficient/developing/satisfactory/unsatisfactory counts, tier-labelled via `SA.getTierLabel`), `fail_count_pre_penalty` / `fail_rate_pre_penalty` (unsatisfactory tier).
  - **Marking-behaviour:** `within_student_sd_pct` (mean across students of each student's SD across their per-criterion `finalScore`s), `late_penalty_count`, `override_count` / `override_rate`, `notes_completion_rate` (trimmed `markerNotes` > 5 chars), `feedback_word_count_mean` / `_sd`.
  - **Reliability:** `alpha` (Cronbach's α; null unless n≥8 **and** ≥2 criteria with full rows).
  - **PER-CRITERION: read but NOT emitted.** Per-criterion `finalScore`/`midpoint` rows are consumed twice — folded into `within_student_sd_pct` (across-criteria SD, *per student*, then averaged) and into the α covariance — but the function emits **no per-criterion aggregate**: no per-criterion mean, no per-criterion band/tier histogram, no per-criterion override tally. **FK-12's literal worked example ("criterion band histogram chip") is therefore the one signal the engine does NOT already produce** — it would need a new aggregation pass (cheap: the rows are already looped for SD/α at `:142–150` and `:218–224`, so a per-criterion tally rides the existing iteration).
  - **PER-TUTOR: no standing breakdown.** `detectState(cohort, currentTutor)` (`:261–292`) classifies mixed-marker **State A** (single-marker), **B** (multi + every `tutor` non-blank), **C** (multi + any blank/`nan` tutor), and *can* return a `subset_students` filtered to one tutor — but per-tutor **metrics** exist only if `cohortMetrics` is re-run on that subset. The cross-tutor / role-label analysis is explicitly deferred to a "Coordinator Dashboard" (see disclaimers at `:582`). **In the app today `renderInsights` is always called with `currentTutor=''` (scorer.html:3104), so the State-B tutor-subset path is effectively dead** — every rendered figure is whole-cohort.

  **Q2 · Reusable-by-design, destination-only-in-practice.** `cohortMetrics` and `detectState` are **pure** — they take plain args (`config`, `students` / `cohort`, `currentTutor`), read no DOM, write no storage, and are exported on `window.CohortInsights`. They are fully reusable in-flow. **But the sole consumer anywhere is `renderInsights`**, invoked once from the Cohort Insights modal (`scorer.html:3104`, via `_renderCohortInsights`); `cohortMetrics`/`detectState` are never called directly outside that view. So the DoD line *"reuses insights stats (no duplicate computation)"* is satisfiable as-is: FK-12 calls `CohortInsights.cohortMetrics(config, cohort.students)` — no new engine.
  - **Input-seam constraint (the real FK-12 design fork):** the engine reads **saved cohort records** (`s.scoreResult`, `s.markerNotes`, `s.feedbackText`). The in-progress student is **not** in `cohort.students` until `saveCurrentStudentToCohort`. An ambient in-flow indicator therefore reflects only **already-saved** scripts unless FK-12 synthesises a live record from the current `scoreResult` and appends it before calling `cohortMetrics`.
  - **Small-N suppression:** `SMALL_N=20`, `VERY_SMALL_N=12`, `alpha_minimum_n=8`, bimodality n<6 → unflagged, skew n<3 / kurtosis n<4 → null. Early in a marking session the shape stats are suppressed or hedged ("too few scripts to interpret reliably"), so an ambient chip is blank/low-confidence for roughly the first ~12 saved scripts.

- **Consequence (feeds FK-12):** INS-7 clears FK-12's **technical** gate — the metrics engine is pure, public, and reusable in-flow with no duplicate computation. Two scope forks remain (both for the feature worktree, not here):
  1. **Indicator-source fork.** Reuse an *existing* aggregate (scale-use badge, `grade_bands` sparkline, `total_score_sd` / `within_student_sd_pct`, running mean) → **S, no engine change**; OR add the per-criterion tally the card's histogram example needs → **M, small additive pass on the rows already iterated**. The card's example is not free; the cohort-level numbers are.
  2. **Dataset fork.** Saved-cohort-only (trivial — call `cohortMetrics` on `cohort.students`) vs live-augmented (append a synthetic record from the in-progress `scoreResult`, touching the recalculate path).
  - **Product risk is unchanged and is NOT an inspection blocker:** the anchoring concern (an in-flow signal biasing markers toward the mean) is real — note the existing module already hedges *every* rendered string ("describes the cohort, not your marking", "not a performance review", `:582` disclaimer). An always-visible running-mean chip is exactly that vector. The card's own DoD (behind a settings toggle, **default off**, self-pilot before any default-on) is the mitigation; it stays. The remaining open question — *does an ambient signal help or cause anchoring* — is answerable only by the self-pilot, not by code inspection, so it travels as an implementation/DoD gate, not as a reopened INS.
  - **Free finding (not carded):** `renderInsights`' `currentTutor` arg is hard-wired to `''` at the only call site, so the State-B per-tutor subset is dead in the app today. If FK-12 (or a later card) ever wants per-tutor ambient signals, that wiring must be supplied first — flagged here so it isn't rediscovered cold.
...
- **Findings — Q1/Q2 (☑ 2026-06-13, Phase-3 close — full ARIA-attribute grep + JS setter read; 54 ARIA occurrences in scorer.html across 11 modals, 2 landmark regions, a toast, and the validation/focus paths):**

  **Q1 · Validation state is set PER-WIDGET — but the divergence is deliberate, not accidental sprawl.** Two independent inline setters with *intentionally different* semantics, no shared helper:
  - **`#grade-override` (overall letter override) — HARD invalid.** `_renderOverrideStatus()` (scorer.html:1788–1815): not in the active grade scale → `aria-invalid='true'` + red `style.borderColor` + red status text in the `#grade-override-status` live region; in-scale → `aria-invalid='false'` + green. Genuine validity signalling.
  - **per-criterion `override-<i>` numeric inputs — SOFT warning, aria-invalid deliberately suppressed.** Set inside `recalculate()` (1916–1931): out-of-band → `.out-of-band` **class only**; `aria-invalid` is forced to `'false'` in **both** branches (oob and in-band). The out-of-band state is conveyed visually but **never** as `aria-invalid='true'`. **This is exactly the commit FK-13's rationale cited** ("remove aria-invalid from out-of-band override warning") — out-of-band was *intentionally demoted* from invalid to soft-warn, because an out-of-band override is a legitimate marker choice, not a data error.
  - **Consequence:** there is no central `setFieldValidity()` model — each widget hand-rolls `class` + `style.borderColor` + status text + `aria-invalid`. But the hard-invalid vs soft-warn split is a **correct semantic distinction**; a forced central helper that flattened it would be a *regression*. Two lesser smells ride along: (a) the channel is inconsistent — the overall override signals via inline hex `borderColor` (`#16a34a`/`#dc2626`), the per-criterion via a CSS class (FK-16 styling-consolidation territory); (b) neither override input carries `aria-describedby` linking it to its status text, so the status string isn't programmatically associated with the field.

  **Q2 · Live regions — focus-mode nav IS announced; the recomputed score is NOT.**
  - **Focus-mode navigation: COVERED.** `#focus-live` (sr-only, `aria-live='polite'`, scorer.html:570) is written on every focus criterion change (`focusOnGrade`/render, 2256–2260): `"<dir> — Criterion i of n: <name>."` with step direction. Focus-workbench navigation announcements exist and read sensibly.
  - **Score updates: GAP (the one substantive finding).** The recomputed weighted total (`weighted-total-cell`, 1949), penalised score (`penalised-score-display`, 1952) and per-row `final-score-<i>`/`weighted-<i>` cells (1910–1912) are plain `.textContent` writes **outside any live region**. A screen-reader user who changes a grade hears nothing about the resulting total or banded grade unless they manually navigate to the cell — the core marking feedback loop (change grade → total/grade recompute) is **silent to assistive tech**. Maps to **WCAG 2.1 AA 4.1.3 Status Messages** (the value is in the DOM but not announced). The only score-adjacent announcements today are `#grade-override-status` (manual override only) and the penalty conflict/info regions (`penalty-override-conflict` role=alert, `penalty-override-info`) — none fire on an ordinary grade change.
  - **Scaffolding otherwise healthy (context, not gaps):** all 11 modals carry `role='dialog' + aria-modal='true' + aria-labelledby`; sticky action bar (1197) and footer (4907) have `role='region' + aria-label`; `#fk-dispatch-toast` is `role='status' aria-live='polite' aria-atomic`; `#focus-toggle` tracks `aria-pressed`; collapsible panels track `aria-expanded`/`aria-controls`. Landmark/modal/announcement structure is in place (consistent with FK-17 having cleared the axe `region`/dialog inventory).

- **Verdict (resolves the gate — SPLIT, leaning "narrow card, not a centralization refactor"):**
