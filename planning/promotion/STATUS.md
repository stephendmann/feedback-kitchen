...
| Push / `gh pr create` | ✅ Executed 2026-06-12 on user confirmation (full snapshot kept, two-stage sequencing): **PR #20** (FK-01 tests) opened and **merged**; **PR #21** (Phase-0 UI bundle, 4 commits, base main) **merged 2026-06-12** (`15c9281`) — https://github.com/stephendmann/feedback-kitchen/pull/21. **Production verified live**: marking.stephendmann.com/scorer.html serves the de-lettered rail (`data-rail="student"`, no letter values) and `.btn-danger`. Phase 0 fully shipped: code + Addendum F + INS-3 map + planning snapshot all on main. |
